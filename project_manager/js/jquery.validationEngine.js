...
(function ($) {
    ...
    var methods = {
            ...
            if (form.data('jqv') === undefined || form.data('jqv') == null) {
                ...
                $(".formError").live("click", function () {
                    $(this).fadeOut(150, function () {
                        ...
                        //$(this).remove();
                        var formName = $(this).attr('class').split('parentForm', 2)[1].split(' ', 1)[0];
                ...
                $(".fieldError").live('focus', function () {
                    var id = $(this).attr("id");
                    $("#" + id + "_formError").show();
                    $("#" + id + "_formError").animate({"opacity": 0.87});
                });
                $(".fieldError").live('blur', function () {
                    var id = $(this).attr("id");
                    $("#" + id + "_formError").hide();
                    $("#" + id + "_formError").animate({"opacity": 0});
                });
            }
        },
        /**
         ...
         * Need to look into these methods for not removing the attribute.
         * to show it again on mouseover
         */
        hidePrompt: function () {
            var promptClass = "." + $(this).attr("id").replace(":", "_") + "formError"
            $(promptClass).fadeTo("fast", 0, function () {
                //$(this).remove();
            });
        },
        /**
        ...
        hide: function () {
            ...
            $('.' + formParentalClassName).fadeTo("fast", 0, function () {
                //$(this).remove();
            });
        },
        /**
        ...
        _validateField: function (field, options, skipAjaxFieldValidation) {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                if (errorMsg !== undefined && promptText === "") {
        ...
        _buildPrompt: function (field, promptText, type, ajaxed, options) {
            //alert(field+'->'+promptText+'->'+type+'->'+ajaxed+'->'+options);exit;
            //alert(field.attr("id"));
            // create the prompt
            field.addClass("fieldError");
            var prompt = $('<div>');
            prompt.addClass("hideerror");
            //prompt.attr("style","display:none;");
            prompt.attr("id", field.attr("id") + "_formError");
