...
class HotelController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Hotel']))
		{
			...
			$model->thumbnail = isset($_FILES['Hotel']['name']['thumbnail']) && $_FILES['Hotel']['name']['thumbnail']!=''  ? $_FILES['Hotel']['name']['thumbnail'] : '';
			...
			if($model->thumbnail !='')
			{
			if(!is_dir("images/hotel/"))
														{
										
														 mkdir("images/hotel/", 0777);
														}
														
				$model->thumbnail=CUploadedFile::getInstance($model,'thumbnail');
				$model->thumbnail->saveAs(Yii::app()->basePath . '/../../images/hotel/'. $model->thumbnail );			
					
				// $image = Yii::app()->image->load(c);			
                // $image->resize(247, 219);
                // $image->save();
				
				$image = new EasyImage(Yii::app()->basePath . '/../../images/hotel/'. $model->thumbnail);
				$image->resize(220, 149);
				$image->save(Yii::app()->basePath . '/../../images/hotel/'. $model->thumbnail);
			}	
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Hotel']))
		{
			$model->attributes=$_POST['Hotel'];
			$model->date_modified = date("Y-m-d H:i:s");
			$model->thumbnail = isset($_FILES['Hotel']['name']['thumbnail']) && $_FILES['Hotel']['name']['thumbnail']!=''  ? $_FILES['Hotel']['name']['thumbnail'] : $thumbnail;
			...
			if($model->save())
			{
						if($model->thumbnail !='' && $model->thumbnail != $thumbnail)
						{
							...
							// $image = Yii::app()->image->load(Yii::app()->basePath . '/../../images/hotel/'. $model->thumbnail );			
							// $image->resize(247, 219);
							// $image->save();
							$image = new EasyImage(Yii::app()->basePath . '/../../images/hotel/'. $model->thumbnail);
							$image->resize(220, 149);
