...
class Packages extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('package_name, category_id,pricing, category_name,show_on_site, package_day, package_night, destination, key_feature, package_thumbnail, date_added, date_modified', 'required'),
			array('category_id', 'numerical', 'integerOnly'=>true),
			array('package_thumbnail', 'file','types'=>'jpg, gif, png'),
			array('package_name, category_name, destination, key_feature, package_thumbnail', 'length', 'max'=>500),
			array('package_day, package_night', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, package_name, category_id, category_name, package_day, package_night, destination, key_feature, package_thumbnail, date_added, date_modified', 'safe', 'on'=>'search'),
		);
	}
