...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	*/ ?>
	<tr>
		<td><?php 
		echo $form->hiddenField($model, 'Artist_Id'); 
		echo $form->labelEx($model,'Artist_Name'); ?></td>
		<td>
		
		
		<?php /*<?php echo $form->textField($model,'Artist_Name',array('size'=>60,'maxlength'=>150)); ?>
		<?php echo $form->error($model,'Artist_Name'); ?> */ ?>
		
		<?php 
		$this->widget('zii.widgets.jui.CJuiAutoComplete', array(
			'name'=>'model_tblaaartist[Artist_Name]',
           ...
           $('#label').val(ui.item.label);
            ...
            $('#call_code').val(ui.item.call_code);
						 $('#model_tblaaartist_Artist_Id').val(ui.item.id);
