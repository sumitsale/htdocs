...
<script type="text/javascript">
			...
			//alert(param);return false;
			   ajaxRequest = $.ajax({
			  url: "<?php echo CController::createUrl("Comments/artist"); ?>",
			  type: 'POST',
			  data: param,
			  error: function(){
				alert('error');
			...
			//alert(param);return false;
			   ajaxRequest = $.ajax({
			  url: "<?php echo CController::createUrl("Comments/statuschange"); ?>",
			  type: 'POST',
			  data: param,
			  error: function(){
				alert('error');
...
<table width="100%">
			...
			<tr>	
				...
				<td><?php if($result[$i]['artist_id']== ''){echo "No Data";}
							...
							$artistName=Yii::app()->db2->createCommand()
								 ->select('*')
								 ->from('TBL_ARTISTS')
								 ->where('ARTIST_ID=:artist_id',array(':artist_id'=>$result[$i]['artist_id']))
								 ->queryAll();
			...
			?></td>
				...
				/*
				  echo Chtml::link('Approve','', array('submit'=>array('Comments/statuschange'),
				  ...
				  ?><a href="javascript:void()" onClick="statuschange(<?php echo $result[$i]['comment_id']; ?>,'<?php echo $result[$i]['comment_status']; ?>');">
						...
						{/*
						 echo Chtml::link('Disapprove','', array('submit'=>array('Comments/statuschange'), 
						 'params'=>array('id'=>$result[$i]['comment_id'],'status'=>$result[$i]['comment_status']),
						 'style'=>'cursor: pointer; text-decoration: none;',));*/
						 ?>
						 <a href="javascript:void()" onClick="statuschange(<?php echo $result[$i]['comment_id']; ?>,'<?php echo $result[$i]['comment_status']; ?>');">
				...
				<td>
				<?php /*echo Chtml::link('Delete','', array('submit'=>array('Comments/artist'),
