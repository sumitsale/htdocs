...
$(document).ready(function() {
	...
	var dataString = 'dir='+ dir;
												...
												$.ajax({  
															type: "POST",  
															url: "<?php echo CController::createUrl("Topartists/sort");  ?>	",  
															data: dataString,
...
echo $form->labelEx($model,'artist_name'); ?>
<?php //echo $form->error($model,'student_name');
           ...
           $('#label').val(ui.item.label);
            ...
            $('#call_code').val(ui.item.call_code);
						 $('#Topartists_artist_id').val(ui.item.id);
...
for($i=$sample[0]-1;$i<$j;$i++){
 ...
 ?>   			
				...
				<tr>
					...
					<td align='left'><?php echo $result[$i]['artist_name']; ?></td> 			
					<td align='center'><?php echo date("M d, Y",$result[$i]['indate']);?></td>
					<td align="center">	
		
					<?php echo CHtml::link('UP', '#', array('class'=>'report','id'=>'up-'.$result[$i]['artist_id']."-".($z-1))); ?> &nbsp;
					<?php echo CHtml::link('DOWN', '#', array('class'=>'report','id'=>'down-'.$result[$i]['artist_id']."-".($z+1))); ?> 
					</td> 
					
				
			
					<td align="center">				
					<?php echo Chtml::link('Delete','#', array('submit'=>array('delete'),
					'params'=>array('id'=>$result[$i]['artist_id']),
