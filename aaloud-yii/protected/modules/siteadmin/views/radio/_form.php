...
<input type="hidden" name="content_id" value="<?php echo $contentid; ?>">
	...
	<tr align="center">
		<td></td>
		<td><?php echo CHtml::submitButton($model->isNewRecord ? 'Add to RadioPlayer' : 'Save'); ?></td>
	</tr>
...
for($i=$sample[0]-1;$i<$j;$i++){
 ...
 ?>   			
				<tr>
					...
					<td align="center">				
					<?php echo CHtml::link('UP', CController::createUrl("radio/priority?ppriority=up&id=".$result[$i]['PLAYERID']."&priority=".($z-1)), array('class'=>'report')); ?> &nbsp;
					<?php echo CHtml::link('DOWN', CController::createUrl("radio/priority?ppriority=down&id=".$result[$i]['PLAYERID']."&priority=".($z+1)), array('class'=>'report')); ?> 
					</td> 
						 
					
			
					<td align="center">				
					<?php echo Chtml::link('Delete','', array('submit'=>array('radio/del'), 'params'=>array('id'=>$result[$i]['PLAYERID']), 'style'=>'cursor: pointer; text-decoration: none;',)); ?> 
					</td> 
