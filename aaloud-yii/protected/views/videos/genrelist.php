<?php $this->pageTitle = "Music Video Genre"." | Video Genre "." | India Artist "."| Music Promotion - "." ArtistAloud.com"; ?>

<?php Yii::app()->clientScript->registerMetaTag('Watch music videos of artists and bands by music genres and get to listen to the latest songs of their genres only at ArtistAloud.com','description'); ?>
<?php Yii::app()->clientScript->registerMetaTag('music video, lyrics, genres, independent artist videos, artist genre, video genre, genre list, music artists, genre artist, songs','keywords'); ?>
...
<div class="content-left fl">
                ...
                <div class="page-title mt10">
                	<h2 class="page-title-block fl">Video Genres</h2>
 ...
 <div class="panel mt10">
		...
		<?php echo CHtml::link(''.$genre[$i]['GENRE_NAME'], CController::createUrl("videos/genredirectory/id/".$genre[$i]['GENRE_ID']), array('class'=>'lang')); ?>
		
        <div class="img-list">
        	<ul id=list-nav>
					
							<?php 
								if(isset($allgenreimages[$i]) && !(empty($allgenreimages[$i])))
							{
							$imagenumber='';
							if(count($allgenreimages[$i])>2)
							{
							$imagenumber=count($allgenreimages[$i])-1;
							}
							else
							{
							$imagenumber=count($allgenreimages[$i]);
							}
							
							
								for($v=0;$v<$imagenumber;$v++)
									{
									 if (file_exists(Yii::app()->basePath . '/../xml/content/videos/' . "video-" . $allgenreimages[$i][$v]['CONTENT_ID'].".xml")) 
									 {
							   $videofile = simplexml_load_file(Yii::app()->basePath . '/../xml/content/videos/' . "video-" . $allgenreimages[$i][$v]['CONTENT_ID'].".xml"); 
							...
							<?php if($image !='') { ?>
								 ...
								 <li><a href="<?php echo CController::createUrl('videos/index', array('id'=>$allgenreimages[$i][$v]['CONTENT_ID'],'name'=>str_replace(' ','-',$allgenreimages[$i][$v]['CONTENT_TITLE']))); ?>" ><img src="<?php echo $image; ?>"/></a></li>
							<?php } else { ?>
								 <li><a href="<?php echo CController::createUrl('videos/index', array('id'=>$allgenreimages[$i][$v]['CONTENT_ID'],'name'=>str_replace(' ','-',$allgenreimages[$i][$v]['CONTENT_TITLE']))); ?>" ><img src="<?php echo  Yii::app()->baseUrl."/themes/aaloud/images/Thumbnails/80x50.png"; ?>"/></a></li>
        ...
        <div class="more">
        	<!--<a href="">More</a>-->
					<?php if(isset($allgenreimages[$i][2])) { ?>
					
			<?php echo CHtml::link('More', $this->createUrl('videos/genredirectory',array(
