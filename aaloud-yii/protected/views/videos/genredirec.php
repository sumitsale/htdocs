		...
		function link(cur)
		{
			...
			var dataString = 'currentPage='+ newValue;
										$.ajax({  
															type: "POST",
															async:false,															
															url: "<?php echo CController::createUrl("videos/ajaxGenre");  ?>	",  
															data: dataString,															
...
<?php $this->pageTitle =$genre[0]['GENRE_NAME']." Music Videos |".$genre[0]['GENRE_NAME']. " Song Videos". " | Music Videos Genre"." - ArtistAloud.com";  ?>

<?php Yii::app()->clientScript->registerMetaTag("Watch ".$genre[0]['GENRE_NAME']." music videos of ".$genre[0]['GENRE_NAME']." artists only at ArtistAloud.com",'description'); ?>
<?php Yii::app()->clientScript->registerMetaTag($genre[0]['GENRE_NAME']." music video, ".$genre[0]['GENRE_NAME']." songs videos,".$genre[0]['GENRE_NAME']." music,".$genre[0]['GENRE_NAME']." genre, Indian music, songs, independent artist, bands, artistaloud",'keywords'); ?>
...
<div class="content-left fl">
								...
								<?php echo CHtml::link('Genre Listing', CController::createUrl("videos/genrelist"), array('class' => 'breadcrumb grey99 fnt11')); ?> &#x21d2;
								<?php echo $genre[0]['GENRE_NAME']; ?> Videos
								</div>

    <div class="page-title mt10">
        <h2 class="page-title-block fl"><?php echo $genre[0]['GENRE_NAME']; ?> Videos</h2>
    ...
    <div class="top-buttons">
        ...
        <div class="buttons">
                ...
                <li><a class="trigger-l" title="languages"><span class="font-mole">Languages</span></a></li>
								<li><a class="current trigger" title="categories" ><span class="font-mole">Genres</span></a></li>
            </ul>
                ...
                <div class="panal-list">
				 <ul class="each-list">
								<?php for($i=0;$i < count($genrelist);$i++) { ?>
											...
											<li><?php echo CHtml::link(''.$genrelist[$i]['GENRE_NAME'], CController::createUrl("videos/genredirectory/id/".$genrelist[$i]['GENRE_ID'])); ?></li>
            ...
            <div class="panal-content">
                ...
                <?php for($i=0;$i<count($langlist);$i++) { ?>
                          ...
                          <li><?php echo CHtml::link(''.$langlist[$i]['LANGUAGE_TITLE'], CController::createUrl("videos/langdirectory/id/".$langlist[$i]['LANGUAGE_ID'])); ?></li> 
                    <?php if(($i+1)%8==0) { ?>
      ...
      //window.onload=playStream;
            ...
            <div class="main-video">
			  <div id="player"></div>
						...
						<?php 
									$name='';
									if(isset($firstvideo->names->artistName) && $firstvideo->names->artistName)
									{
									$size=count($firstvideo->names->artistName);
									for($q=0;$q<$size;$q++)
											{
											foreach($firstvideo->names->artistName[$q]->attributes() as $a => $b)
												{					 
													if($a=='id' && $b=='31')
														{
														$name = $firstvideo->names->artistName[$q]->name;
														}
												}
											} 
								  }
        ...
        <div class="desc">
            <div>
						<?php if(!(empty($firstvideo->videoName))) { ?>
                <h3 class="singer-name"><?php echo $name; ?> - <?php echo $firstvideo->videoName; ?></h3>
						<?php } ?>
            </div>

            <div>
						<?php if(!(empty($firstvideo->genre))) { ?>
                <h4 class="genre-name"><?php echo $firstvideo->genre; ?></h4>
						<?php } ?>
            </div>
            ...
            <div class="grey99 para">
						<?php if(!(empty($firstvideo->videodetail->videoDescription))) { ?>
                <p><?php echo $firstvideo->videodetail->videoDescription; ?></p>
						<?php } ?>
            </div> 
        ...
        <div class="page-title mt10">
            ...
            <h3 class="sub-page-title-block fl"><?php echo $row['GENRE_NAME']; ?> Videos</h3>
			...
			$videolisting =  simplexml_load_file(Yii::app()->basePath . '/../xml/content/videos/'."video-" . $result[$i]['CONTENT_ID'] . ".xml");
											$videoPath = '';
											  ...
											  for ($l = 0; $l < count($videolisting->paths->path); $l++) {
												...
												if ($videolisting->paths->path[$l]->attributes() == '32') {
                            ...
                            <div class="vid-bottom">
                                ...
                                <div class="name-list">
                                    ...
                                    <div class="song fnt11 grey99" style="cursor:pointer;">
                                        <?php $str = $videolisting->videoName;
																							if(strlen($str)>20)
																							{
